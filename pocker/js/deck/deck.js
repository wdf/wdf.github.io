...
var Deck = (function () {
  ...
  function _card(i) {
    ...
    function setSide(newSide,rank,suit) {
      // flip sides
      if (newSide === 'front') {
        if (self.side === 'back') {
          $el.removeChild($back);
        ...
        self.side = 'front';
        $el.appendChild($face);
        self.setRankSuit((rank === undefined)?self.rank:rank, (suit === undefined)?self.suit:suit);
      } else {
        if (self.side === 'front') {
          $el.removeChild($face);
        }
  ...
  var poker = {
    deck: function deck(_deck4) {
      var postions = [{x: -130, y: 140, r: 0, xo: 65}, {x: 100, y: -160, r: -360, xo: 10}, {x: -90, y: -160, r: 360, xo: -10}];
      ...
      function poker(next, data) {
        ...
        __fontSize = fontSize();
        var size = data.cards.length * data.total;
        cards.slice(size * -1).reverse().forEach(function (card, i) {
          var currentIndex = i % data.total;
          var postion = postions[currentIndex];
          card.poker(i, len,postion, function (i,rank, suit) {
              if(rank){
                  card.setSide('front',rank, suit);
              }
            if (i === size-1) {
              next();
            }
    ...
    card: function card(_card4) {
      ...
      _card4.poker = function (i, len,postion, cb,data) {
        var delay = i * 250;Math.round((i - 2.05) * 70 * __fontSize / 16),
        console.log(__fontSize / 16);
        var s = Math.round(i / data.total);
        _card4.animateTo({
          delay: delay,
          duration: 250,
          x: Math.round((postion.x +( s * postion.xo)) * __fontSize / 16),
          y: Math.round(postion.y   * __fontSize / 16),
          rot:postion.r,
          ...
          onComplete: function onComplete() {
              var c = data.cards[s];
              if (i % data.total === data.current - 1) {
                  cb(i,c[0],c[1]);
              }else{
                  cb(i);
              }
